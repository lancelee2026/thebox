...
场景设置位于选关面板底部。「高空模式」是主开关，隐藏树脂托盘和舞台外框；「云雾挑战」只有在高空模式开启后才可操作。挑战云由同一透明素材裁出三种云团轮廓，以远、中、近三层随机选择左右入口、高度、速度和间隔：远云位于棋盘后方，中近云从前景短暂遮挡路径，并通过尺寸、透明度与轻微景深虚化建立空间关系。云雾跨场为缓慢、低压的景深运动，近云也不得形成快速掠过感。两项开关默认关闭并保存在本地；关闭高空模式会同步关闭云雾挑战。开关使用原生 checkbox/switch 语义、清晰说明和 48×28px 轨道。
...
橙色脆弱砖在长方体竖立承重而破裂时，会先分成四片并下沉，再进入失败重开；这段反馈与角色失败并行，不改变判定、步数或重试节奏。减少动态效果时保留短距离分离下沉，取消连续旋转。

前景云素材 `src/assets/cloud-far.png`、`cloud-mid.png` 与 `cloud-near.png` 于 2026-09-01 通过 OpenAI imagegen 从原云带重新提取，并按 Alpha 连通区域拆成三张独立图片；不使用矩形蒙版、切片或几何裁切。生成要求为“三种成熟休闲游戏白色积云、真正透明背景、完整羽化边缘、无矩形雾底/文字/UI/角色/水印”。高空模式下云层活动范围越过正方形舞台，但 HUD 与操作区保持在云层上方；WebGL 画布使用 1.5 倍超采样留白并反向补偿相机缩放，使棋盘大小不变、坠落实体可越过原舞台边界。素材仅用于玩家主动开启的视觉挑战，不参与碰撞判定。
