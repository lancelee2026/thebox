...
前景云素材 `src/assets/cloud-far.png`、`cloud-mid.png` 与 `cloud-near.png` 于 2026-09-01 通过 OpenAI imagegen 从原云带重新提取，并以本地确定性 Alpha 裁切拆成三张独立图片。生成要求为“三种成熟休闲游戏白色积云、真正透明背景、完整羽化边缘、无矩形雾底/文字/UI/角色/水印”；交付素材均保留独立透明留白，运行时只做等比例缩放、换形和随机穿场。素材仅用于玩家主动开启的视觉挑战，不参与碰撞判定。
