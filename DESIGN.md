...
**The Molded Stage Rule.** 默认模式下路径不能直接漂在纯色卡片上；主舞台必须保留可读的边框厚度、内层色差和落地阴影。只有玩家主动开启高空模式后，才允许托盘与外框退场；路径砖仍必须保留轻阴影和蓝灰侧面。
...
场景设置位于选关面板底部。「高空模式」是主开关，隐藏树脂托盘和舞台外框；「云雾挑战」只有在高空模式开启后才可操作，以约 19 秒一轮的前景云短暂遮挡路径。背景云层另以 46 秒周期做极慢幅度漂移。两项开关默认关闭并保存在本地；关闭高空模式会同步关闭云雾挑战。开关使用原生 checkbox/switch 语义、清晰说明和 48×28px 轨道。
