...
选关面板提供「高空模式」和从属的「云雾挑战」，存储键为 `thebox:scene-prefs:v1`。首次进入默认同时开启两者；已有用户保存的显式开关优先。`Game.ts` 负责读取、约束与持久化；`LevelView.setHighAltitude()` 只控制托盘 mesh 的退场；`setup.ts` 切换 WebGL 透明底、雾，并在高空中彻底关闭接影面。高空模式把 WebGL 画布扩为 1.5 倍并反向补偿相机缩放，为坠落保留真实的舞台外渲染空间，默认模式仍使用原正方形画布。棋盘 framing 使用非线性缓和曲线：短关收敛角色与格子的屏幕比例，长关仍优先完整呈现路径。`ui/cloudField.ts` 管理远、中、近三层云的随机换形、方向、高度、速度和可见性；云图来自独立 Alpha 连通区域，不使用矩形蒙版；远云 DOM 层位于 WebGL 画布后方，中近云位于前方，页面隐藏、开关关闭或减少动态效果时会停止穿场动画。无 Web Animations API 时云改用 CSS transition 穿场；无 `matchMedia` 时按非减少动态处理。Web Audio 不可用时音效静默降级，不阻断操作。两项设置不得进入 `WorldSnapshot`，也不得影响规则、撤销、步数和星级。
